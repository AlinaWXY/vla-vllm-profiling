...
from profiling.runtime import offline_assets, require_headroom, stream_checkpoint_weights, buffered_checkpoint_weights
...
def main():
    ...
    p.add_argument("--weights-cache", type=Path, help="Existing verified BF16 serialization; original config/checkpoint identity retained")
    p.add_argument("--expected-fingerprint", type=Path, help="Original-checkpoint full_scopes.json; require exact loaded parameter identity")
    p.add_argument("--buffered-original", action="store_true", help="Sequential buffered I/O for original F32 weights on slow NFS")
    p.add_argument("--launch-config", type=Path, help="Recorded Inductor launch choices to hold fixed across NCU application passes")
    ...
    p.add_argument("--scope", choices=("expert", "all"), default="expert")
    p.add_argument("--profile-kind", choices=("kernels", "graphs", "ranges"), default="kernels")
    p.add_argument("--legacy-image-keys", action="store_true",
                   help="Reproduce the historical missing-camera bug ONLY for controlled diagnosis")
    ...
    args = p.parse_args()
    frozen_source = os.environ.get("PI05_PROFILE_SOURCE")
    if frozen_source and Path(__file__).resolve() != Path(frozen_source).resolve() / "profiling/bench_pi05.py":
        raise RuntimeError("Requested frozen harness was shadowed by another checkout")
    if args.weights_cache and not args.expected_fingerprint:
        p.error("A derived weight cache requires the original model fingerprint")
    def phase(message):
    ...
    if args.launch_config:
        from profiling.freeze_launches import install
        install(args.launch_config)
    ...
    class LocalPipeline(Pi05Pipeline):
        ...
        def _load_checkpoint(self, model):
            ...
            def weights():
                ...
                iterator = (buffered_checkpoint_weights(args.weights_cache) if args.weights_cache else
                            buffered_checkpoint_weights(self.model_dir, original=True) if args.buffered_original else
                            stream_checkpoint_weights(self.model_dir))
    ...
    robot_obs = {"prompt": "pick up the red block and place it in the bin",
                 ...
                 "images": {(k.removeprefix("observation.images.") if args.legacy_image_keys else k):
                            rng.integers(0, 256, (224, 224, 3), dtype=np.uint8) for k in camera_keys}}
    if args.legacy_image_keys:
        input_audit = {"all_requested_cameras_present": False, "matched_camera_count": 0,
                       "warning": "Historical bug reproduction: shortened image keys mask all cameras; text/state-only prefix"}
        phase(input_audit["warning"])
    else:
    ...
    with torch.inference_mode(), strict_checkpoint_load(Pi05ForActionPrediction) as load_audit:
        ...
        input_audit.update(image_masks=actual_masks, prefix_metadata=checked[4],
                           language_valid_tokens=int(checked[3].sum().item()))
        if args.legacy_image_keys:
            if any(actual_masks):
                raise RuntimeError("Legacy reproduction unexpectedly accepted a camera")
        elif len(actual_masks) != args.cameras or not all(actual_masks):
            raise RuntimeError(f"Refusing missing-camera benchmark: {actual_masks}")
        del checked
        phase(f"Validated input cameras: {input_audit}")
        ...
        if not args.profile and args.scope == "expert":
        ...
        if args.scope == "all":
            from profiling.full_vla import run
            report = run(pipeline, robot_obs, optimized_req, optimized_actions, observed, args)
            report.update(load_audit=load_audit, input_audit_preflight=input_audit,
                          checkpoint=str(args.checkpoint), seed=args.seed, steps=args.steps,
                          packages=packages, torch=torch.__version__, cuda=torch.version.cuda,
                          host=platform.node(), memory_before=memory_before)
            report["launch_config_path"] = str(args.launch_config) if args.launch_config else None
            report["weight_cache"] = str(args.weights_cache) if args.weights_cache else None
        ...
        input_audit["expert_prefix_len"] = kwargs.get("prefix_len")
        input_audit["expert_valid_prefix_len"] = kwargs["valid_prefix_len"]
        if not args.legacy_image_keys and kwargs["valid_prefix_len"] != input_audit["prefix_metadata"]["prefix_valid_len"]:
            raise RuntimeError("Expert prefix does not match the validated multimodal input")
