...
def run(pipeline, robot_obs, optimized_req, expected_pipeline_actions, observed, args):
    ...
    def vlm():
        # Preserve the PR's existing image graph semantics and compiled kernels.
        # Masks/position tensors are static metadata, prepared outside this scope.
        with label("vlm"):
            embs = []
            for index, pixels in enumerate(images):
                with label(f"vlm.vision.camera{index}"):
                    image_emb = model._run_image_embed(pixels, cuda_graph_image_embed=True,
    ...
    np.savez(args.output / "actions.npz", optimized=expected_pipeline_actions,
             scoped=actual.float().cpu().numpy(), noise=noise.cpu().numpy())
