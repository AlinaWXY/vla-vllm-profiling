...
def main():
    ...
    original_image_embed = model._run_image_embed

    def image_snapshot(*a, **kw):
        return original_image_embed(*a, **kw).clone()

    def images_eager(*a, **kw):
        kw.update(cuda_graph_image_embed=False, torch_compile_image_embed=False,
                  torch_compile_image_embed_fullgraph=False)
        return original_image_embed(*a, **kw)
    ...
    with torch.inference_mode():
        for case in first["cases"]:
            ...
            # A graph output is reused in place. Check whether the three camera
            # entries retained by embed_prefix alias the same mutable storage.
            from vllm_omni.diffusion.models.pi05.processor_pi05 import build_model_inputs
            pixels = build_model_inputs(obs, pipe.config, pipe.tokenizer, pipe._device,
                                        max_cameras=3)[0]
            eager_images = [images_eager(img) for img in pixels]
            graph_images, saved_images = [], []
            for img in pixels:
                embedding = original_image_embed(img, cuda_graph_image_embed=True,
                    torch_compile_image_embed=True, torch_compile_image_embed_fullgraph=True)
                graph_images.append(embedding)
                saved_images.append(embedding.clone())
            image_audit = {
                "all_camera_outputs_share_storage": len({x.data_ptr() for x in graph_images}) == 1,
                "retained_outputs_vs_last_camera": [error(x.float().cpu().numpy(),
                    saved_images[-1].float().cpu().numpy()) for x in graph_images],
                "snapshot_vs_eager_per_camera": [error(x.float().cpu().numpy(),
                    y.float().cpu().numpy()) for x, y in zip(saved_images, eager_images)],
                "overwritten_vs_snapshot_per_camera": [error(x.float().cpu().numpy(),
                    y.float().cpu().numpy()) for x, y in zip(graph_images, saved_images)],
            }
            for snapshot in (False, True):
                def graph_image(*a, **kw):
                    kw.update(cuda_graph_image_embed=True, torch_compile_image_embed=True,
                              torch_compile_image_embed_fullgraph=True)
                    value = original_image_embed(*a, **kw)
                    return value.clone() if snapshot else value
                label = "safe_graph_image_snapshot" if snapshot else "safe_graph_images"
                with diagnosis.replace(model, "_run_image_embed", graph_image):
                    value = run("safe")
                metrics[label] = error(value, reference)
                arrays[name+"__"+label] = value
            ...
            for mode in ("original", "rope_model", "rope_model_euler", "rope_model_eager_euler",
                         "rope_model_image_snapshot", "rope_model_images_eager"):
                ...
                with ExitStack() as stack:
                    if "image_snapshot" in mode or "images_eager" in mode:
                        stack.enter_context(diagnosis.replace(model, "_run_image_embed",
                            image_snapshot if "image_snapshot" in mode else images_eager))
                    if "rope" in mode:
                        stack.enter_context(diagnosis.replace(rt, "_matmul_rope_qkv",
                            diagnosis.LaunchAdapter(rope_eager if "eager_euler" in mode else rope)))
                    if "euler" in mode:
